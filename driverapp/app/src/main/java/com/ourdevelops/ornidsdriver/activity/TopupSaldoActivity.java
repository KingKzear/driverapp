...
public class TopupSaldoActivity extends AppCompatActivity {
    ...
    private void sheetlist() {
        ...
        if (sp.getSetting()[22].equals("2")) {
            ...
            i.putExtra("nominal", convertAngka(nominal.getText().toString() + ".00"));
        }
        i.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
        startActivity(i);
    }
    ...
    private void getPaypal() {
        if (sp.getSetting()[22].equals("2")) {
            Amount = Double.parseDouble(convertAngka(nominal.getText().toString().replace(sp.getSetting()[4], "")));
        } else {
            Amount = Double.parseDouble(convertAngka(nominal.getText().toString().replace(sp.getSetting()[4], "")) + "00");
        }
        ...
        if (requestCode == PayUmoneyFlowManager.REQUEST_CODE_PAYMENT && resultCode == RESULT_OK && data !=
            ...
            if (transactionResponse != null && transactionResponse.getPayuResponse() != null) {
                if (transactionResponse.getTransactionStatus().equals(TransactionResponse.TransactionStatus.SUCCESSFUL)) {
                    ...
                    notif.message = getString(R.string.topup_success);
